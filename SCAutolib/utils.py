...
def _gen_private_key(key_path: Path, size: int = 2048):
    ...
    :param size: The size of the key to be created.
    :type size: int
    :return: None
    """
...
def _install_packages(packages):
    """
    Installs a list of specified DNF packages on the system.
...
def load_token(card_name: str = None, update_sssd: bool = False):
    ...
    :param update_sssd: A boolean flag. If ``True``, the SSSD configuration
                        file will be updated with a ``shadowutils`` rule for
                        the user of the loaded card.
...
def ca_factory(path: Path = None, cnf: OpensslCnf = None,
    ...
    :param create: A boolean flag. If ``True``, a new CA object will be created
