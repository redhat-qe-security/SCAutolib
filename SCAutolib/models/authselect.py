...
with specified features (see man authselect-migration(7)).
"""
...
class Authselect:
    ...
    def __init__(self, required=False, lock_on_removal=False,
        ...
        :param required: A boolean flag. If ``True``, the
                         ``with-smartcard-required`` option will be added to
                         the ``authselect`` profile.
        :type required: bool
        :param lock_on_removal: A boolean flag. If ``True``, the
                                ``with-smartcard-lock-on-removal`` option will
                                be added to the ``authselect`` profile.
        :type lock_on_removal: bool
        :param mk_homedir: A boolean flag. If ``True``, the ``with-mkhomedir``
                           option will be added to the ``authselect`` profile,
                           ensuring home directories are created on login
                          .
        :type mk_homedir: bool
        :param sudo: A boolean flag. If ``True``, the ``with-sudo`` option will
