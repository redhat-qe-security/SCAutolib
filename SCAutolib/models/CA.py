"""
This module implements classes that represent Certification Authorities (CA)
...
class BaseCA:
    ...
    shared methods for CA-related operations, especially managing the system's
    CA trust database (``sssd_auth_ca_db.pem``).
    """
    dump_file: Path = None
    ca_type: str = None
    _ca_cert: Path = None
...
class LocalCA(BaseCA):
    ...
    def request_cert(self, csr: Path, username: str,
        ...
        :param cert_out: An optional ``pathlib.Path`` object specifying where
                         the signed certificate should be duplicated. It can be
