...
the parent (File) class that is supposed to operate on general config file. Only
config files containing sections are supported by File class as config parser
that is used to manipulate files does not support section-less configs.
...
class File:
    ...
    Note:   Only files compatible with ConfigParser are supported (i.e. files
            containing sections)
    """
    _conf_file = None
    _template = None
    ...
    def set(self, key: str, value: Union[int, str, bool], section: str = None):
        ...
        """
        if section is None:
            raise TypeError(f"argument 'section' not provided")
