    ...
    * clean:  remove config file
    ...
    def create(self):
        ...
        :raises FileExistsError: If the configuration file already exists on
                                 the file system.
        :raises FileNotFoundError: If no template file was provided during
...
class OpensslCnf(File):
    ...
    different types of CNF files (e.g., for CAs, for users) by utilizing
    specific templates and performing string replacements.
    """
    _template = None
    _conf_file = None
    _content = None
    ...
    def __init__(self, filepath: Union[str, Path], conf_type: str,
        ...
        :param filepath: The path where the OpenSSL CNF file will be saved.
        :type filepath: Union[str, pathlib.Path]
        :param conf_type: An identifier string indicating the type of CNF file
                          (e.g., `"CA"` or `"user"`), which determines the
                          template and replacement patterns.
        :type conf_type: str
    ...
    def create(self):
        ...
        content to be written to the actual CNF file.
    ...
    def save(self):
        ...
        parser if ``set`` was used) to the OpenSSL CNF file on the file system.
