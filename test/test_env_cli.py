...
def test_prepare_fail_config(config_file_incorrect, caplog):
...
def test_cleanup(real_factory, loaded_env, caplog, runner):
    ...
    data["restore"].append({"type": "user", "username": "test-user"})
    ...
    # Run cleanup command
    try:
        check_output(["useradd", "test-user"])
        ...
        check_output(["userdel", "test-user", "-r"])
        raise
